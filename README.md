...
- 항목 우클릭: 부가설명 추가, 내일로 이월 또는 삭제
- 부가설명은 개수·줄 수 제한 없이 작성
- 부모 항목 가운데 버튼 드래그: 부가설명과 함께 순서 변경
- 부가설명 가운데 버튼 드래그: 같은 항목 안에서 순서 변경
- `Ctrl+Z` / `Ctrl+Shift+Z`: 실행 취소·다시 실행
- `Ctrl+X` / `Ctrl+C` / `Ctrl+V`: 잘라내기·복사·붙여넣기
- `Esc`: 달력으로 돌아가기
- 시스템 설정에 맞춘 다크 모드 자동 대응
- 창 크기 조절, SQLite 자동 저장, 마지막 창 위치·크기 복원, Pretendard 10pt 적용
